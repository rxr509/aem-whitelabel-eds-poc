...
| `brands/<id>/` | that brand's overrides | **yes** |

`shared/` is copied first, then `brands/<id>/` is overlaid on top, so a file
present in both comes from the brand folder. Today exactly three paths collide
for the default brand, `acme`:
...
five files under `styles/` and the brand folder has two, the result has five,
with two overwritten. A brand can override or add files, but cannot delete a
file that `shared/` ships — see known-issues.md #1, which matters for the
...
| Branch | Role |
|---|---|
| `source` | where humans author. Holds `brands/`, the `shared` submodule, `scripts/`, `docs/` and `.github/`. |
...
Each brand is a folder under `brands/`: `brands/acme/`, and so on. Only one
brand is served in production at a time — `acme` — but the build and the local
preview can target any brand folder on disk by ID:

```sh
npm run build:site -- --brand=acme
npm run dev -- --brand=acme
```

`--brand` defaults to `acme` when omitted, so existing behaviour and
`build-and-sync.yml` (which passes no flag) are unchanged. An unknown ID fails
immediately, before any worktree or build work happens, and reports the path it
looked for and the brand IDs that exist:

```
build-site: no such brand "foo" - looked for /path/to/repo/brands/foo
build-site: available brands: acme
```

This is single-repo, single-active-brand scoping only — there is no cross-repo
deploy and no per-brand branch or EDS registration. Adding a second brand for
real (its own `develop`, its own live site) is a separate, larger change.
...
```sh
npm run build:site                    # assemble the default brand (acme)
npm run build:site -- <dir>           # assemble into <dir>
npm run build:site -- --brand=<id>    # assemble a different brand
```
...
`.github/workflows/build-and-sync.yml` calls this same script with no `--brand`
flag, then commits and pushes the result to `develop`. Because both paths run
identical code, what you see locally (for the default brand) is what gets
published.
...
While `npm run dev` runs, edits to `shared/` or the active brand folder are
copied into the preview automatically — an incremental copy, not a full rebuild.

Override precedence is preserved. The watcher resolves which layer owns a path
on every event, checking the brand folder first regardless of which tree
changed, so:

- editing a `shared/` file that the brand folder shadows leaves the preview
  alone — the override is not clobbered
- deleting a brand override falls back to the `shared/` version
...
```sh
npm run watch:site                  # ../aem-poc-preview, brand acme
npm run watch:site -- <dir>
npm run watch:site -- --brand=<id>
```
...
- Un-ignoring a directory un-ignores everything inside it, including nested
  ...
  not catch one dropped inside `brands/` or `docs/`.

## Linting

```sh
npm run lint        # stylelint over brands/acme CSS, eslint over brands/acme JS
npm run lint:fix
```
